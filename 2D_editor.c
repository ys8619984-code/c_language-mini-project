...
/* Helper and Drawing Functions defined topologically */
...
void render_all_shapes(char canvas[ROWS][COLS], ShapeNode* head) {
    reset_canvas(canvas);
    for (ShapeNode* cur = head; cur; cur = cur->next) {
        switch (cur->type) {
            case LINE: draw_line(canvas, cur->data.line.x1, cur->data.line.y1, cur->data.line.x2, cur->data.line.y2); break;
            case RECTANGLE: draw_rectangle(canvas, cur->data.rect.x1, cur->data.rect.y1, cur->data.rect.x2, cur->data.rect.y2); break;
            case CIRCLE: draw_circle(canvas, cur->data.circle.xc, cur->data.circle.yc, cur->data.circle.r); break;
            case TRIANGLE: draw_triangle(canvas, cur->data.tri.x1, cur->data.tri.y1, cur->data.tri.x2, cur->data.tri.y2, cur->data.tri.x3, cur->data.tri.y3); break;
        }
    }
}
...
void list_shapes(ShapeNode* head) {
    ...
    for (ShapeNode* temp = head; temp; temp = temp->next) {
        switch (temp->type) {
            case LINE: printf("ID %d: Line (%d,%d)->(%d,%d)\n", temp->id, temp->data.line.x1, temp->data.line.y1, temp->data.line.x2, temp->data.line.y2); break;
            case RECTANGLE: printf("ID %d: Rect (%d,%d) to (%d,%d)\n", temp->id, temp->data.rect.x1, temp->data.rect.y1, temp->data.rect.x2, temp->data.rect.y2); break;
            case CIRCLE: printf("ID %d: Circle C(%d,%d) R:%d\n", temp->id, temp->data.circle.xc, temp->data.circle.yc, temp->data.circle.r); break;
            case TRIANGLE: printf("ID %d: Tri (%d,%d), (%d,%d), (%d,%d)\n", temp->id, temp->data.tri.x1, temp->data.tri.y1, temp->data.tri.x2, temp->data.tri.y2, temp->data.tri.x3, temp->data.tri.y3); break;
        }
    }
}
...
void save_drawing(ShapeNode* head, const char* filename) {
    ...
    for (ShapeNode* t = head; t; t = t->next) {
        ...
        switch (t->type) {
            case LINE: fprintf(fp, " %d %d %d %d\n", t->data.line.x1, t->data.line.y1, t->data.line.x2, t->data.line.y2); break;
            case RECTANGLE: fprintf(fp, " %d %d %d %d\n", t->data.rect.x1, t->data.rect.y1, t->data.rect.x2, t->data.rect.y2); break;
            case CIRCLE: fprintf(fp, " %d %d %d\n", t->data.circle.xc, t->data.circle.yc, t->data.circle.r); break;
            case TRIANGLE: fprintf(fp, " %d %d %d %d %d %d\n", t->data.tri.x1, t->data.tri.y1, t->data.tri.x2, t->data.tri.y2, t->data.tri.x3, t->data.tri.y3); break;
        }
    }
    fclose(fp);
...
void load_drawing(ShapeNode** head, int* next_id, const char* filename) {
    ...
    while (fscanf(fp, "%d %d", &id, &type) == 2) {
        ...
        switch (type) {
            case LINE: fscanf(fp, "%d %d %d %d", &node->data.line.x1, &node->data.line.y1, &node->data.line.x2, &node->data.line.y2); break;
            case RECTANGLE: fscanf(fp, "%d %d %d %d", &node->data.rect.x1, &node->data.rect.y1, &node->data.rect.x2, &node->data.rect.y2); break;
            case CIRCLE: fscanf(fp, "%d %d %d", &node->data.circle.xc, &node->data.circle.yc, &node->data.circle.r); break;
            case TRIANGLE: fscanf(fp, "%d %d %d %d %d %d", &node->data.tri.x1, &node->data.tri.y1, &node->data.tri.x2, &node->data.tri.y2, &node->data.tri.x3, &node->data.tri.y3); break;
        }
...
void input_shape_coords(ShapeNode* node, int is_modify) {
    switch (node->type) {
        case LINE:
            printf(is_modify ? "New coords (X1 Y1 X2 Y2): " : "Enter coords (X1 Y1 X2 Y2): ");
            scanf("%d %d %d %d", &node->data.line.x1, &node->data.line.y1, &node->data.line.x2, &node->data.line.y2);
            break;
        case RECTANGLE:
            printf(is_modify ? "New coords (X1 Y1 X2 Y2): " : "Enter top-left & bottom-right coords (X1 Y1 X2 Y2): ");
            scanf("%d %d %d %d", &node->data.rect.x1, &node->data.rect.y1, &node->data.rect.x2, &node->data.rect.y2);
            break;
        case CIRCLE:
            printf(is_modify ? "New parameters (XC YC R): " : "Enter center coords & radius (XC YC R): ");
            scanf("%d %d %d", &node->data.circle.xc, &node->data.circle.yc, &node->data.circle.r);
            break;
        case TRIANGLE:
            printf(is_modify ? "New vertices (X1 Y1 X2 Y2 X3 Y3): " : "Enter three corners (X1 Y1 X2 Y2 X3 Y3): ");
